...
class OverlordTask(threading.Thread):
    ...
    def __init__(self, zk_manager, context, port_for_client, port_for_node, zookeeper_hosts):
        threading.Thread.__init__(self)
        self.__zk_manager = zk_manager
...
class OverlordRebalanceTask(threading.Thread):
    ...
    def __select_move_keys(self, src_keys, target_keys, src_value, target_value):
        ...
        target_hashed = []
        for k in src_keys:
            src_hashed.append((Hash.get_hashed_value(k, self.__hash_range), k))
        for k in target_keys:
            target_hashed.append((Hash.get_hashed_value(k, self.__hash_range), k))
...
class Overlord:
    ...
    def start_overlord(self, config_path, port_for_client, port_for_node, zookeeper_hosts, hash_range, max_slave_count):
        ...
        context = zmq.Context()
        self.__overlord_task = OverlordTask(zk_manager, context, config['port_for_client'], config['port_for_node'], config['zookeeper_hosts'])
